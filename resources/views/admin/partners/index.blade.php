                ...
                <div class="card-body">
                    <form id="createThisForm" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" id="image" name="image" class="form-control" accept="image/*" onchange="previewImage(event, '#preview-image')">
...
$(function() {
    ...
    function previewImage(event, selector){ let output = $(selector); output.attr('src', URL.createObjectURL(event.target.files[0])).show(); }
    ...
    $('#addBtn').click(function() {
        var btn = this;
        var url = $(btn).val() === 'Create' ? "{{ route('partners.store') }}" : "{{ route('partners.update') }}";
    ...
    $(document).on('click', '.EditBtn', function() {
        var id = $(this).data('id');
        $.get("{{ url('/admin/partner') }}/" + id + "/edit", {}, function(res) {
            $('#codeid').val(res.id);
            $('#name').val(res.name);
            if (res.image) $('#preview-image').attr('src', '/images/partners/' + res.image).show();
    ...
    $(document).on('change', '.toggle-status', function() {
        ...
        $.post("{{ route('partners.toggleStatus') }}", {id: id, status: status}, function(res){
