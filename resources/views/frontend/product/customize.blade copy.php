...
<head>
  ...
  <title>Product Customiser — Slim + Inspector + Views</title>
  ...
  <style>
    ...
    .top-strip{ background:var(--black); color:var(--white); padding:6px 0; font-size:14px; }
    .top-strip .item { display:flex; align-items:center; gap:.6rem; justify-content:center; }
    .top-strip i { color:var(--red); }

    .progress-wrapper { background:#fff; padding:12px 0; border-bottom:1px solid #eee; }
    .checkout-progress { display:flex; gap:24px; align-items:center; justify-content:center; flex-wrap:wrap; }
    .stage { text-align:center; color:#333; width:190px; }
    .stage .circle { width:46px; height:46px; border-radius:50%; display:flex; align-items:center; justify-content:center; background:#111; color:#fff; border:3px solid #222; margin:0 auto 8px; }
    .stage.active .circle { background:var(--red); border-color:var(--red); }
    .stage.complete .circle { background:#198754; border-color:#198754; }

    .main { padding:22px 0; }
    .card-panel { border-radius:10px; background:#fff; border:1px solid #e9e9e9; padding:14px; }
    .card-panel-dark { border-radius:10px; background:var(--panel); color:var(--white); padding:14px; border:1px solid rgba(255,255,255,0.06); }

    .preview-shell { background:#f7f7f7; border-radius:10px; padding:12px; min-height:520px; display:flex; align-items:center; justify-content:center; position:relative; overflow:hidden; }
    .preview-canvas{ width:320px; height:420px; position:relative; border-radius:10px; background-size:cover; background-position:center; box-shadow:0 18px 40px rgba(13,20,30,0.08); transform-origin:center center; }
    .layer-item { position:absolute; transform-origin:center center; display:flex; align-items:center; justify-content:center; pointer-events:auto; cursor:grab; background:transparent; overflow:visible; }
    .layer-item img { max-width:100%; max-height:100%; display:block; user-select:none; -webkit-user-drag:none; }
    .layer-text { white-space:nowrap; display:inline-block; padding:2px 4px; }

    .option-card { border-radius:8px; border:1px solid #ececec; padding:10px; cursor:pointer; transition:all .12s ease; background:#fff; }
    .option-card.active { border-color:var(--red); box-shadow:0 8px 24px rgba(220,32,38,0.08); transform:translateY(-4px); }
    .small-muted { color:var(--muted); font-size:13px; }

    .toolbar { display:flex; gap:8px; align-items:center; flex-wrap:wrap; }
    .zoom-controls { display:flex; gap:6px; align-items:center; }

    .loader-overlay { position:absolute; inset:0; display:flex; align-items:center; justify-content:center; background:rgba(11,11,11,0.6); color:#fff; z-index:999; display:none; flex-direction:column; gap:10px; border-radius:10px; }

    .inspector { margin-top:12px; background:#fff; border-radius:8px; padding:10px; border:1px solid #eee; }
    .inspector h6 { margin:0 0 8px 0; font-size:14px; }

    @media (max-width:991px){
      .preview-shell { min-height:420px; }
      .preview-canvas { width:260px; height:360px; }
    }
    @media (max-width:575px){
      .preview-shell { min-height:360px; padding:8px; }
      .preview-canvas { width:220px; height:300px; }
      .stage { width:130px; font-size:12px; }
    }
    .btn-red { background:var(--red); color:#fff; border:none; }
    .btn-red:hover { background:#b51f31; }

    .layer-item.selected { outline: 3px dashed rgba(220,32,38,0.45); }
    .list-group-item.active { background: linear-gradient(90deg, rgba(220,32,38,0.08), rgba(220,32,38,0.02)); border-color: #f0c0c2; }
    .view-btns .btn { min-width:76px; }
  </style>
</head>
<body>
  ...
  <div class="top-strip">
    <div class="container">
      <div class="row text-center gx-2 gy-1 align-items-center">
        <div class="col-4 item"><i class="fa-solid fa-truck-fast"></i> Next day delivery</div>
        <div class="col-4 item"><i class="fa-solid fa-box-open"></i> No Minimum Order</div>
        <div class="col-4 item"><i class="fa-solid fa-gift"></i> Free Delivery Over <strong>£99.00</strong></div>
      </div>
    </div>
  </div>
  ...
  <div class="progress-wrapper">
    <div class="container">
      <div class="checkout-progress" role="navigation" aria-label="Progress">
        <div class="stage complete" data-stage="1"><div class="circle">1</div><p>Choose product</p></div>
        <div class="stage active" data-stage="2"><div class="circle">2</div><p>Assign customisations</p></div>
        <div class="stage inactive" data-stage="3"><div class="circle">3</div><p>Design & preview</p></div>
      </div>
    </div>
  </div>
          ...
          <div class="card-panel">
            ...
            <div id="panels">
              <div class="mb-3">
                ...
                <div class="row g-2">
                  ...
                  <div class="col-12 col-md-12">
                    <div class="option-card product-option active" data-product='{"id":"tshirt","name":"T-Shirt","img":{"front":"https://images.unsplash.com/photo-1541099649105-f69ad21f3246?q=80&w=800&auto=format&fit=crop&s","back":"https://images.unsplash.com/photo-1541099649105-f69ad21f3246?q=80&w=800&auto=format&fit=crop&s","left":"https://images.unsplash.com/photo-1541099649105-f69ad21f3246?q=80&w=800&auto=format&fit=crop&s","right":"https://images.unsplash.com/photo-1541099649105-f69ad21f3246?q=80&w=800&auto=format&fit=crop&s"},"baseWidthCm":30}'>
                      <div class="d-flex align-items-center">
                        <div class="me-2"><i class="fa-solid fa-shirt fa-2x" style="color:var(--red)"></i></div>
                        <div><div class="fw-bold">T-Shirt</div><div class="small-muted">From £9.99</div></div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="mb-3">
                ...
                <div class="row g-2">
                  <div class="col-md-6">
                    <div class="option-card method-option active" data-method='{"id":"print","label":"Print","maxWidthCm":30,"setup":5.99}'>
                      <div class="d-flex align-items-start">
                        <div class="me-2"><i class="fa-solid fa-print fa-2x" style="color:var(--red)"></i></div>
                        <div><div class="fw-bold">Print</div><div class="small-muted">Max Width: 30cm · Setup £5.99</div></div>
                      </div>
                    </div>
                  </div>
                  <div class="col-md-6">
                    <div class="option-card method-option" data-method='{"id":"embroidery","label":"Embroidery","maxWidthCm":25,"setup":9.99}'>
                      <div class="d-flex align-items-start">
                        <div class="me-2"><i class="fa-solid fa-pen-nib fa-2x" style="color:var(--red)"></i></div>
                        <div><div class="fw-bold">Embroidery</div><div class="small-muted">Max Width: 25cm · Setup £9.99</div></div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="mb-3">
                <h6>3. Choose position</h6>
                <div class="row g-2">
                  <div class="col-6 col-md-4 col-lg-3 mb-2"><button class="btn btn-outline-dark w-100 pos-btn" data-pos="right-chest">Right Chest</button></div>
                  <div class="col-6 col-md-4 col-lg-3 mb-2"><button class="btn btn-outline-dark w-100 pos-btn active" data-pos="centre-chest">Centre Chest</button></div>
                  <div class="col-6 col-md-4 col-lg-3 mb-2"><button class="btn btn-outline-dark w-100 pos-btn" data-pos="left-chest">Left Chest</button></div>
                  <div class="col-6 col-md-4 col-lg-3 mb-2"><button class="btn btn-outline-dark w-100 pos-btn" data-pos="top-chest">Top Chest</button></div>
                  <div class="col-6 col-md-4 col-lg-3 mb-2"><button class="btn btn-outline-dark w-100 pos-btn" data-pos="centre-back">Centre Back</button></div>
                  <div class="col-6 col-md-4 col-lg-3 mb-2"><button class="btn btn-outline-dark w-100 pos-btn" data-pos="top-back">Top Back</button></div>
                  <div class="col-6 col-md-4 col-lg-3 mb-2"><button class="btn btn-outline-dark w-100 pos-btn" data-pos="shoulder-blades">Shoulder Blades</button></div>
                  <div class="col-6 col-md-4 col-lg-3 mb-2"><button class="btn btn-outline-dark w-100 pos-btn" data-pos="bottom-back">Bottom Back</button></div>
                  <div class="col-6 col-md-4 col-lg-3 mb-2"><button class="btn btn-outline-dark w-100 pos-btn" data-pos="left-sleeve">Left Sleeve</button></div>
                  <div class="col-6 col-md-4 col-lg-3 mb-2"><button class="btn btn-outline-dark w-100 pos-btn" data-pos="right-sleeve">Right Sleeve</button></div>
                </div>
                <div class="mt-2 small-muted">Selected position sets where newly-added layers will be placed. You can add multiple layers and set different positions per-layer.</div>
              </div>
              ...
              <div class="mb-3">
                ...
                <div class="row g-2">
                  <div class="col-md-6">
                    ...
                    <input class="form-control" id="addImagesInput" type="file" accept="image/*" multiple>
                    <div class="small-muted mt-1">Supported: PNG, JPG. Max per file: 6 MB.</div>
                  </div>

                  <div class="col-md-6">
                    <label class="form-label">Add text</label>
                    <input class="form-control" id="addTextInput" placeholder="Enter text (press Add)"/>
                    <div class="d-flex gap-2 mt-2 align-items-center">
                      <select id="fontFamily" class="form-select form-select-sm" style="max-width:170px">
                      ...
                      <div class="input-group input-group-sm" style="max-width:140px">
                        <input id="fontSizeInput" type="number" class="form-control" value="28" min="8" max="200">
                        <span class="input-group-text">px</span>
                      </div>

                      <div class="btn-group" role="group" aria-label="Font styles">
                        <button id="boldToggle" class="btn btn-outline-secondary btn-sm" title="Bold"><i class="fa-solid fa-bold"></i></button>
                        <button id="italicToggle" class="btn btn-outline-secondary btn-sm" title="Italic"><i class="fa-solid fa-italic"></i></button>
                        <button id="underlineToggle" class="btn btn-outline-secondary btn-sm" title="Underline"><i class="fa-solid fa-underline"></i></button>
                      </div>
                    </div>

                    <div class="d-flex gap-2 align-items-center mt-2">
                      <input id="textColorInput" type="color" value="#000000" title="Text color">
                      <div class="d-flex gap-1 ms-2">
                        <button class="btn btn-sm btn-outline-dark color-swatch" data-color="#000000" style="background:#000;color:#fff"></button>
                        <button class="btn btn-sm btn-outline-dark color-swatch" data-color="#ffffff" style="background:#fff"></button>
                        <button class="btn btn-sm btn-outline-dark color-swatch" data-color="#dc2026" style="background:var(--red)"></button>
                        <button class="btn btn-sm btn-outline-dark color-swatch" data-color="#1b9dd9" style="background:#1b9dd9"></button>
                      </div>
                      <button id="addTextBtn" class="btn btn-red btn-sm ms-auto">Add Text</button>
                    </div>

                    <div class="small-muted mt-2">You can add multiple images and text layers. Use the Layers panel below to edit/delete specific layers.</div>
                  </div>
                </div>
              </div>
              ...
              <div class="mb-3">
                ...
                <div id="layersList" class="list-group mb-2" style="max-height:220px; overflow:auto;"></div>
                <div class="d-flex gap-2">
                  <button id="deleteLayerBtn" class="btn btn-outline-danger btn-sm ms-auto" title="Delete selected layer"><i class="bi bi-trash"></i> Delete</button>
                </div>
        ...
        <div class="col-lg-4">
          <div class="card-panel-dark position-relative">
            ...
            <div class="d-flex justify-content-between align-items-center mb-2">
              ...
              <div class="d-flex gap-2 align-items-center">
                <div class="zoom-controls">
                  <button id="zoomOut" class="btn btn-sm btn-outline-light" title="Zoom out"><i class="bi bi-zoom-out"></i></button>
                  <div class="input-group input-group-sm" style="width:72px;">
                    <input id="zoomLevel" type="text" class="form-control text-center" value="100%" readonly>
                  </div>
                  <button id="zoomIn" class="btn btn-sm btn-outline-light" title="Zoom in"><i class="bi bi-zoom-in"></i></button>
                </div>

                <div class="ms-2">
                  <button id="downloadPngBtn" class="btn btn-sm btn-light" title="Download PNG"><i class="bi bi-download"></i></button>
                </div>
              </div>
            </div>
            ...
            <div class="mt-3" id="previewMeta">
              <div class="small-muted">Product: <span id="currentProductName" class="fw-bold">T-Shirt</span></div>
  ...
  class ProductCustomiserSlimViews {
    constructor() {
      ...
      const defaultProduct = firstProductEl ? JSON.parse(firstProductEl.getAttribute('data-product')) : {
        ...
        img:{ front:'', back:'', left:'', right:'' }, baseWidthCm:30
      };

      this.state = {
        product: defaultProduct,
        method: { id:'print', label:'Print', maxWidthCm:30, setup:5.99 },
    ...
    initUI() {
      ...
      $('#boldToggle,#italicToggle,#underlineToggle').on('click', function(){ $(this).toggleClass('active'); });
      $('#previewShell').on('click', function(){ $(this).focus(); });
    }
    ...
    bindProductOptions() {
      document.querySelectorAll('.product-option').forEach(el=>{
        el.addEventListener('click', ()=> {
          document.querySelectorAll('.product-option').forEach(x=>x.classList.remove('active'));
      ...
      input.addEventListener('change', (e) => {
        ...
        files.forEach(file => {
          ...
          reader.onload = (ev) => {
            ...
            this.addLayer({ type: 'image', src, pos: this.state.position, widthPx: approxWidth, heightPx: null, rotate: 0, opacity: 1, borderRadiusPx: 0, bgColor: 'transparent', draggable: true, editable: true, view: this.currentView });
          };
    ...
    bindAddText() {
      document.getElementById('addTextBtn').addEventListener('click', () => {
        ...
        const fontSize = Math.max(8, parseInt(document.getElementById('fontSizeInput').value || 28));
    ...
    bindZoomControls() {
      ...
      document.getElementById('zoomOut').addEventListener('click', () => this.setZoom(Math.max(0.3, this.state.zoom - 0.1)));
    ...
    initTooltips() {
      ...
      tipTriggerList.forEach((el) => new bootstrap.Tooltip(el, { container: 'body' }));
    }
    ...
    selectLayer(id) {
      ...
      Array.from(this.layersList.children).forEach(li => li.classList.toggle('active', li.getAttribute('data-id') === id));
      Array.from(this.previewCanvas.querySelectorAll('.layer-item')).forEach(n => n.classList.toggle('selected', n.dataset.id === id));
      this.showInspectorFor(id);
    }

    showInspectorFor(id) {
      const layer = this.state.layers.find(l => l.id === id);
      if (!layer) { this.hideInspector(); return; }
    ...
    hideInspector() {
      ...
      this.selectedLayerId = null;
      Array.from(this.previewCanvas.querySelectorAll('.layer-item.selected')).forEach(n => n.classList.remove('selected'));
    }

    showLayerEditor(id) {
      const layer = this.state.layers.find(l => l.id === id);
      if (!layer) return;
      if (!layer.editable) { alert('This layer is locked for editing'); return; }
    ...
    exportPNG() {
      ...
      Array.from(this.previewCanvas.querySelectorAll('.layer-item.selected')).forEach(el => el.classList.remove('selected'));
      html2canvas(this.previewCanvas, { scale: 2, backgroundColor: null }).then(canvas => {
    ...
    recalcPrice() {
      const basePriceMap = { 'tshirt': 9.99, 'mug': 6.99, 'bag': 8.99 };
    ...
    render() {
      ...
      const layers = (this.state.layers || []).slice().filter(l => l.view === this.currentView).sort((a,b) => (a.zIndex||0) - (b.zIndex||0));
      layers.forEach(layer => {
        ...
        if (layer.bgColor && layer.bgColor !== 'transparent') el.style.backgroundColor = layer.bgColor;
        ...
        if (layer.type === 'image') {
          ...
          if (layer.bgColor && layer.bgColor !== 'transparent') span.style.background = layer.bgColor;
        ...
        el.addEventListener('touchstart', (ev) => this.startDrag(ev, layer.id), { passive: true });
    ...
    positionToCss(posKey) {
      const map = {
        ...
        'top-back': { left: 50, top: 10 },
        'shoulder-blades': { left: 50, top: 45 },
    ...
    refreshLayerList() {
      ...
      const layers = (this.state.layers || []).slice().filter(l => l.view === this.currentView).sort((a,b) => (b.zIndex||0) - (a.zIndex||0));
      layers.forEach(l => {
        const li = document.createElement('li');
        li.className = 'list-group-item d-flex justify-content-between align-items-center';
        li.setAttribute('data-id', l.id);
        const title = (l.type === 'image') ? `Image (${l.pos})` : `Text: "${(l.text || '').slice(0,30)}" (${l.pos})`;
        ...
        li.querySelector('.edit-layer').addEventListener('click', (e) => {
          ...
          if (!layer.editable) { alert('Layer is locked for editing'); return; }
    ...
    initLayerDragHandlers() {
      this._drag = { active: false, id: null, startX: 0, startY: 0, initLeftPct: 0, initTopPct: 0, rect: null };
      document.addEventListener('mousemove', (ev) => this.onDragMove(ev));
      document.addEventListener('mouseup', () => this.onDragEnd());
      document.addEventListener('touchmove', (ev) => this.onDragMove(ev), { passive: false });
    ...
    onDragMove(ev) {
      ...
      if (node) { node.style.left = newLeft + '%'; node.style.top = newTop + '%'; }
    }
